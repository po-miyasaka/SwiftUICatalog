...
let swiftAlgorithm: Target.Dependency = .product(name: "Algorithms", package: "swift-algorithms")
let lookingGlassUI: Target.Dependency = .product(name: "LookingGlassUI", package: "LookingGlassUI")

let knocksTarget: [Target] = {
    (1...knocksCount).map {
        Target.target(name: "Knock\($0)", dependencies: [
            Target.Dependency(stringLiteral: "Common"),
            swiftAlgorithm,
            lookingGlassUI,
        ])
    }
...
let knocksTargetDependencies: [Target.Dependency] = (1...knocksCount).map { .init(stringLiteral: "Knock\($0)")}

let package = Package(
    name: "SwiftUICatalogModule",
    platforms: [
        .iOS(.v14)
    ],
    products: [
        ...
        .library(
            name: "SwiftUICatalogModule",
            targets: ["SwiftUICatalogModule"]),
    ],
    dependencies: [
        .package(url: "https://github.com/apple/swift-algorithms.git", .exact("1.1.0")),
        .package(url: "https://github.com/ryanlintott/LookingGlassUI", .exact("0.3.1"))
    ],
    targets: [
        .target(name: "SwiftUICatalogModule", dependencies: knocksTargetDependencies),
        .target(name: "Common")
    ] + knocksTarget
)
