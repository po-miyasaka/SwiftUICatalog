...
public init() {}
    ...
    struct MView: UIViewRepresentable, Equatable, @unchecked Sendable {
        enum Action {
    ...
    class Delegate: NSObject, MKMapViewDelegate {
        let location: @Sendable (CLLocationCoordinate2D) -> Void
        init(location:  @Sendable @escaping (CLLocationCoordinate2D) -> Void) {
            print("init delegate")
