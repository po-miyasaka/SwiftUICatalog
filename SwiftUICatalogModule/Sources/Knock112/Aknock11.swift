...
public struct ContentView: View  {
    ...
    @State var shouldShowPHPView: Bool = false
    @State var hoge = ""
    @State var hoge2 = true
    ...
    public var body: some View {
                ...
                Button(action: {
                    ...
                    Task {
                        withAnimation {
                            ...
                            // TODO: ViewにSendableをつけなくてもここではエラーにならない理由がわからぬ
                        }
                    }
                }, label: {
        ...
        .background(Color.init("chatBackground", bundle: nil).ignoresSafeArea())
...
class ChatViewModel: ObservableObject {
    @Published var sectionDataArray: [Knock112.SectionData] = []
...
struct PHPView: UIViewControllerRepresentable, @unchecked Sendable {
    ...
    final class Coordinator: NSObject, PHPickerViewControllerDelegate, @unchecked Sendable {
        ...
        let handler: @Sendable (URL?) -> Void
        init(handler: @Sendable @escaping (URL?) -> Void) {
        ...
        func picker(_: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
            ...
            result.itemProvider.loadObject(ofClass: UIImage.self, completionHandler: { content, _ in
                let data = (content as? UIImage)?.pngData()
                Task { @MainActor in
                    guard let data else {
...
extension View {
    ...
    func onBubble(_ isRight: Bool, screenSize: CGSize) -> some View {
        ZStack(alignment: isRight ? .topTrailing : .topLeading) {
            foregroundColor(Color.black)
                ...
                .background(
                    SwiftUI.Group {
                        if isRight { SpeechBubble().fill(Color.white).flip(.horizontal) } else { SpeechBubble().fill(Color.white)
                        }
                    }
                )
        }
    ...
    func imageForMessage(_ isRight: Bool, screenSize: CGSize) -> some View {
        frame(width: screenSize.width * 0.5, alignment: isRight ? .trailing : .leading)
            ...
            .padding(isRight ? .trailing : .leading, Knock112.SpeechBubble.xOffset)
